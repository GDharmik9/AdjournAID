# AdjournAI: Architecture Comparison & Concept Guide

This document clarifies and separates the two architectural concepts discussed for **AdjournAI**:
...
* **UI Badges**: Displays *"Verified Grounded"* vs. *"Verification Required"* badges on margin risk cards.
...
This architecture adapts AdjournAI for Google Cloud Platform (GCP) to provide a live deployment URL (`.a.run.app`), satisfy hackathon submission criteria, and leverage Vertex AI tools.
...
* **In your Pitch & Architecture Specs**: Highlight that AdjournAI supports both **Cloud Run + Vertex AI** (for cloud speed) and **SaulLM-7B + ZDR** (for air-gapped enterprise privacy).
